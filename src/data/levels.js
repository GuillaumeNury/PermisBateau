export const levels = [
  ...
  ...addCategory('Alphabet phonétique internationnal', [
    {
      type: "text",
    ...
    },
    {
      ...
      solution: "Zulu",
      choices: randomize(["Zulu", "Zorro", "Zumba", "Zeus"]),
    },
  ]),
]

function addCategory(category, levels) {
  return levels.map(lvl => ({ ...lvl, category }));
}
 ...
 */
function parseExercices(raw, image) {
  return raw
    .replace(/ {4}/g, "")
    .split("\n")
    .map(line => line.split(";"))
...
export const levelsByCategory = levels.reduce(
  (acc, lvl) => ({
    ...acc,
    [lvl.category || 'Autre']: [...(acc[lvl.category || 'Autre'] || []), lvl]
  }),
  {},
);